...
@NamedQueries({
        ...
        @NamedQuery(name = "consultarPorFiltro", query = "select doc, mob, label "
                ...
                + "				) or ( "
                + "					:lotaTitular!=null and :lotaTitular!=0L and doc.lotaCadastrante in (select l.idLotacao from DpLotacao as l where l.hisIdIni = :lotaTitular)"
                + "					or (:titular!=null and :titular!=0L and doc.subscritor in (select p.idPessoa from DpPessoa as p where p.hisIdIni = :titular))"
                + "					or (:titular!=null and :titular!=0L and doc.destinatario in (select p.idPessoa from DpPessoa as p where p.hisIdIni = :titular))"
                + "					or (:lotaTitular!=null and :lotaTitular!=0L and doc.destinatario = null and doc.lotaDestinatario in (select l.idLotacao from DpLotacao as l where l.hisIdIni = :lotaTitular))"
                + "					or (:lotaTitular!=null and :lotaTitular!=0L and label.dpLotacaoIni in (select l.idLotacao from DpLotacao as l where l.hisIdIni = :lotaTitular))"
                + "					or (:titular!=null and :titular!=0L and label.dpPessoaIni.idPessoa in (select p.idPessoa from DpPessoa as p where p.hisIdIni = :titular))"
                + "				)"
                + "			)"
                + "			"
                + "			and ("
                ...
                + "			"
                + "			and (:destinatarioSelId = null or :destinatarioSelId = 0L or doc.destinatario.idPessoa in (select p.idPessoa from DpPessoa as p where p.hisIdIni = :destinatarioSelId))"
                + "			and (:lotacaoDestinatarioSelId = null or :lotacaoDestinatarioSelId = 0L or doc.lotaDestinatario.idLotacao in (select l.idLotacao from DpLotacao as l where l.hisIdIni = :lotacaoDestinatarioSelId))"
                + "			and (:nmDestinatario = null or :nmDestinatario = '' or upper(doc.nmDestinatario) like '%' || :nmDestinatario || '%')"
                + "			"
                + "			and (:cadastranteSelId = null or :cadastranteSelId = 0L or doc.cadastrante.idPessoa in (select p.idPessoa from DpPessoa as p where p.hisIdIni = :cadastranteSelId))"
                + "			and (:lotaCadastranteSelId = null or :lotaCadastranteSelId = 0L or doc.lotaCadastrante.idLotacao in (select l.idLotacao from DpLotacao as l where l.hisIdIni = :lotaCadastranteSelId))"
                + "			"
                + "			and (:subscritorSelId = null or :subscritorSelId = 0L or doc.subscritor.idPessoa in (select p.idPessoa from DpPessoa as p where p.hisIdIni = :subscritorSelId))"
        ...
        @NamedQuery(name = "consultarDocumentosFinalizadosEntreDatas", query = "select doc from ExDocumento doc where "
                ...
                + "					and doc.lotaCadastrante.hisIdIni = :idLotacaoInicial"
                + "					and doc.dtFinalizacao is not null"
