...
@Controller
public class SubstituicaoController extends VraptorController {
	...
	private final List<String> chaves = Prop.getList("/siga.substituto.tipos");
	...
	@Inject
	CpConfiguracaoBL cpConfiguracaoBL;
	...
	private boolean podeCadastrarQualquerSubstituicao() throws Exception {
		return cpConfiguracaoBL.podePorConfiguracao(
	...
	private List<DpSubstituicao> buscarSubstitutos(String substituicao, DpPessoa pessoa, DpLotacao lotacao)
	    ...
	    todasSubst = cpDao.consultarOrdemData(dpSubstituicao);
	    ...
	    for (DpSubstituicao subst : todasSubst) {
	    	...
	    	if (subst.getSubstituto() != null && (subst.getSubstituto().isFechada()
	    			|| cpCompetenciaBL.isPessoaBloqueada(subst.getSubstituto())) && Objects.equals(substituicao, "false"))
	    		continue;
	    	if (subst.isEmVoga() || subst.isFutura()) {
	    		subst.setLotaSubstituto(subst.getLotaSubstituto() != null ? subst.getLotaSubstituto().getHistoricoAtual() : null);
	    		subst.setLotaTitular(subst.getLotaTitular() != null ? subst.getLotaTitular().getHistoricoAtual() : null);
	...
	public void edita(Long id) throws Exception {
		...
		if (id != null) {
			DpSubstituicao subst = cpDao.consultar(id, DpSubstituicao.class, false);
	...
	public void gravar(DpSubstituicao substituicao
		...
		try {
			if (tipoTitular == 1) {
				...
				subst.setTitular(cpDao.consultar(this.titularSel.getId(),DpPessoa.class, false));
				...
				subst.setLotaTitular(cpDao.consultar(this.lotaTitularSel.getId(), DpLotacao.class, false));
			...
			if (substituicao != null && substituicao.getIdSubstituicao() != null) {
				DpSubstituicao substAntiga = cpDao.consultar(substituicao.getIdSubstituicao(), DpSubstituicao.class, false);
				substAntiga.setDtFimRegistro(new Date());
				substAntiga = cpDao.gravar(substAntiga);
				subst.setIdRegistroInicial(substAntiga.getIdRegistroInicial());
			}

			subst = cpDao.gravar(subst);
	...
	public void finalizar() throws Exception {
		try {
			subBl.finalizarSubstituicao(getCadastrante());
		} catch (Exception e) {
		}
	...
	public void substituirGravar(Long id) throws Exception {
		subBl.substituir(getCadastrante(), id);
		String referer = getRequest().getHeader("Referer");
		if (referer != null)
			result.redirectTo(referer);
		else
	...
	public void exclui(Long id) throws Exception {
		...
		if (id != null) {
			DpSubstituicao dpSub = cpDao.consultar(id, DpSubstituicao.class, false);
